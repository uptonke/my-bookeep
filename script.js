...
function isMissingTableError(error) {
  ...
  return error?.code === "PGRST205" || /Could not find the table/i.test(text);
}

async function selectRawTable(name, select = "*") {
  const { data, error } = await state.client.from(name).select(select);
  if (error) return [];
  return data || [];
}

async function fallbackQueryTable(name) {
  if (name === "v_transactions_full" || name === "v_transaction_details") {
    const [txs, accounts, categories, budgetItems] = await Promise.all([
      selectRawTable("transactions"),
      selectRawTable("accounts"),
      selectRawTable("categories"),
      selectRawTable("budget_items")
    ]);
    const byId = rows => new Map(rows.map(r => [r.id, r]));
    const accountMap = byId(accounts);
    const categoryMap = byId(categories);
    const budgetMap = byId(budgetItems);
    return txs.map(t => {
      const account = accountMap.get(t.account_id) || {};
      const toAccount = accountMap.get(t.to_account_id) || {};
      const category = categoryMap.get(t.category_id) || {};
      const budgetItem = budgetMap.get(t.budget_item_id) || {};
      const date = String(t.transaction_date || "");
      return {
        ...t,
        tx_year: Number(date.slice(0, 4)) || null,
        tx_month: Number(date.slice(5, 7)) || null,
        account_name: account.name || "",
        to_account_name: toAccount.name || "",
        category_name: category.name || "未分類",
        budget_item_name: budgetItem.name || "",
  ...
  if (name === "v_account_balances") {
    const [accounts, txs] = await Promise.all([selectRawTable("accounts"), selectRawTable("transactions")]);
    const balances = new Map(accounts.map(a => [a.id, Number(a.initial_balance || 0)]));
    txs.filter(t => t.status !== "cancelled").forEach(t => {
      const amount = Math.abs(Number(t.amount || 0));
      if (t.type === "expense") balances.set(t.account_id, Number(balances.get(t.account_id) || 0) - amount);
      if (t.type === "refund" || t.type === "income") balances.set(t.account_id, Number(balances.get(t.account_id) || 0) + amount);
      if (t.type === "transfer") {
        balances.set(t.account_id, Number(balances.get(t.account_id) || 0) - amount);
        balances.set(t.to_account_id, Number(balances.get(t.to_account_id) || 0) + amount);
      }
      if (t.type === "asset_adjustment") {
        const sign = t.adjustment_direction === "decrease" ? -1 : 1;
        balances.set(t.account_id, Number(balances.get(t.account_id) || 0) + sign * amount);
      }
    });
    return accounts.map(a => ({ ...a, current_balance: Number(balances.get(a.id) || 0) }))
      .sort((a, b) => (a.sort_order || 0) - (b.sort_order || 0) || String(a.name || "").localeCompare(String(b.name || "")));
  }

  if (name === "v_category_spending") {
    const [txs, categories] = await Promise.all([selectRawTable("transactions"), selectRawTable("categories")]);
    const categoryMap = new Map(categories.map(c => [c.id, c]));
    const rows = new Map();
    txs.filter(t => t.status !== "cancelled" && ["expense", "refund"].includes(t.type)).forEach(t => {
      const category = categoryMap.get(t.category_id) || {};
      const year = Number(String(t.transaction_date || "").slice(0, 4)) || null;
      const key = `${year || ""}:${t.category_id || "uncategorized"}:${t.type}`;
      const sign = t.type === "refund" ? -1 : 1;
      const existing = rows.get(key) || { budget_year: year, category_id: t.category_id || null, category_name: category.name || "未分類", type: category.type || "expense", total_amount: 0 };
      existing.total_amount += sign * Math.abs(Number(t.amount || 0));
      rows.set(key, existing);
    });
    return Array.from(rows.values());
  }

  if (name === "v_monthly_cashflow") {
    const txs = await selectRawTable("transactions");
    const rows = new Map();
    txs.filter(t => t.status !== "cancelled").forEach(t => {
      const date = String(t.transaction_date || "");
      ...
      const row = rows.get(key) || { budget_year: year, budget_month: month, income: 0, expense: 0, net: 0 };
      const amount = Math.abs(Number(t.amount || 0));
      if (t.type === "income") row.income += amount;
      if (t.type === "expense") row.expense += amount;
      if (t.type === "refund") row.expense -= amount;
      row.net = row.income - row.expense;
      rows.set(key, row);
    });
    return Array.from(rows.values()).sort((a, b) => a.budget_year - b.budget_year || a.budget_month - b.budget_month);
  }

  if (["v_year_summary", "v_budget_summary", "v_year_budget_summary", "v_budget_item_summary"].includes(name)) {
    return [];
  }
...
async function queryTable(name, options = {}) {
  ...
  if (error) {
    const fallback = isMissingTableError(error) ? await fallbackQueryTable(name) : null;
...
function quickTemplateToAccountMarker(toAccountId = "") {
  return toAccountId ? `\n[bookeep_to_account_id:${toAccountId}]` : "";
}

function readQuickTemplateToAccountFromNote(note = "") {
  const match = String(note || "").match(/\[bookeep_to_account_id:([^\]]+)\]/);
  return match ? match[1] : "";
}

function stripQuickTemplateMarkers(note = "") {
  return String(note || "").replace(/\n?\[bookeep_to_account_id:[^\]]+\]/g, "").trim();
}
...
function activeQuickTemplates() {
  return (state.data.quickTemplates || [])
    ...
    .map(t => ({
      ...t,
      to_account_id: t.to_account_id || readQuickTemplateToAccountFromNote(t.note),
      note: stripQuickTemplateMarkers(t.note || ""),
      key: `custom-${t.id}`,
      is_builtin: false
    }));
}
...
function applyQuickTxTemplate(key) {
  ...
  try {
    ...
    const cleanTemplateNote = stripQuickTemplateMarkers(template.note || "");
    if (form.elements.note && !form.elements.note.value && cleanTemplateNote) form.elements.note.value = cleanTemplateNote;
...
function renderTransactions() {
  ...
  const allRows = applyTxFilters(transactionsForSelectedYear());
  const rows = allRows.slice(0, 20);
  const overflowNote = allRows.length > rows.length
    ? `<div class="empty compact-empty">只顯示最近 20 筆；請用日期、搜尋、分類或帳戶篩選縮小範圍。</div>`
    : "";
  ...
  return `
    ...
    <details class="card collapsible-card">
      <summary class="collapsible-summary">
        ...
        <span class="badge">預覽 ${rows.length} 筆</span>
      </summary>
...
function renderTemplates() {
  const rawEdit = state.editing.quickTemplate;
  const edit = rawEdit ? {
    ...rawEdit,
    to_account_id: rawEdit.to_account_id || readQuickTemplateToAccountFromNote(rawEdit.note),
    note: stripQuickTemplateMarkers(rawEdit.note || "")
  } : null;
  const rows = activeQuickTemplates().sort((a, b) => (a.sort_order || 0) - (b.sort_order || 0) || String(a.name || "").localeCompare(String(b.name || "")));
...
async function writeRow(table, payload, options = {}) {
  let clean = normalizeForWrite(payload);
  ...
  for (let attempt = 0; attempt < 12; attempt += 1) {
    ...
    const missingColumn = missingColumnNameFromError(response.error);
    if (missingColumn && Object.prototype.hasOwnProperty.call(clean, missingColumn)) {
      delete clean[missingColumn];
  ...
  if (removedColumns.length) {
    console.warn(`寫入 ${table} 時略過目前資料庫不存在的欄位：${removedColumns.join("、")}`);
  }

  if (options.expect) {
    for (const [key, expected] of Object.entries(options.expect)) {
      if (removedColumns.includes(key)) continue;
...
async function saveQuickTemplate(form) {
  const d = readForm(form);
  const toAccountId = d.type === "transfer" ? d.to_account_id || null : null;
  const cleanNote = stripQuickTemplateMarkers(d.note || "");
  const payload = {
    id: d.id || undefined,
    name: d.name,
    type: d.type || "expense",
    ...
    note: `${cleanNote}${quickTemplateToAccountMarker(toAccountId)}`.trim(),
    sort_order: Number(d.sort_order || 0),
    is_active: boolValue(d.is_active)
  };
