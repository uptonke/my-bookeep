...
// Bookeep v60 clean bootstrap
// 目標：index.html 只載入 config.js + script.js；舊 hotfix 已整合到這個檔案。

const BOOKEEP_VERSION = "v60";
...
(function patchSupabaseMissingViews() {
  if (!window.supabase || window.supabase.__bookeepV60Patched) return;
  ...
  window.supabase.__bookeepV60Patched = true;
})();
...
function installBookeepV60PostPatches() {
  ...
  document.querySelectorAll(".nav-btn[data-tab]").forEach(btn => {
    if (btn.dataset.v60Bound === "true") return;
    btn.dataset.v60Bound = "true";
  ...
  if (brandVersion) brandVersion.textContent = `雲端資料庫｜${BOOKEEP_VERSION}`;
}

function loadBookeepCore() {
  const script = document.createElement("script");
  script.src = BOOKEEP_CORE_SCRIPT_URL;
  script.defer = false;
  script.onload = () => {
    installBookeepV60PostPatches();
    setTimeout(installBookeepV60PostPatches, 0);
    setTimeout(installBookeepV60PostPatches, 500);
  };
  script.onerror = () => {
...
if (document.readyState === "loading") {
  document.addEventListener("DOMContentLoaded", loadBookeepCore, { once: true });
} else {
  loadBookeepCore();
}
